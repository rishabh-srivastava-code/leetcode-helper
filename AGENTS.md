...
GitHub: https://github.com/rishabh-srivastava-code/
...
3. Clean accepted-style code, preferably in JavaScript unless another language is requested.
...
### Code Quality — **HIGH**
9. [Write Clean JavaScript](#write-clean-javascript)
...
Use the exact method signature from the LeetCode prompt. No type annotations needed in JS.
...
- `const seen = {}`: Creates an object to remember numbers we have already visited.
- `for (let i = 0; i < nums.length; i++)`: Loops through each number with its index.
- `const need = target - nums[i]`: Calculates the number required to form the target.
```
...
**Impact: HIGH** | **Category:** code-quality | **Tags:** javascript, readability

JavaScript solutions should be accepted-style and concise.
...
1. Create an object called `seen` that maps number to index.
2. Loop through `nums`.
...
- `twoSum(nums, target)`: Defines the method with the input array and target value.
- `const seen = {}`: Stores each number we have visited and its index.
- `for (let i = 0; i < nums.length; i++)`: Loops through each number while tracking its index.
- `const need = target - nums[i]`: Calculates the number needed to make the target sum.
...
- Space: `O(n)`, because the object can store up to `n` numbers.

## Edge Cases

- Duplicate numbers: handled because indices are stored separately.
- Negative numbers: handled because subtraction and object lookup work the same way.
